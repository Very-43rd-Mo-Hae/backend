...
This deploy path builds the Spring Boot backend into a Docker image, pushes it to Amazon ECR, then updates Docker Compose on one EC2 host.

## AWS resources

1. Create an ECR private repository, for example `relink-backend`.
...
4. Open the EC2 security group for the public HTTP port you use, usually `8080` or `80` behind a reverse proxy.
...
```bash
sudo mkdir -p /opt/relink/backend
sudo chown "$USER":"$USER" /opt/relink/backend
```

Create `/opt/relink/backend/.env` from `deploy/.env.production.example` and replace every secret value.

The default compose file runs the backend, MySQL, and Redis on the EC2 instance with Docker volumes. To use RDS or ElastiCache later, update `DB_URL`, `BATCH_DB_URL`, and `REDIS_HOST` in `.env`.
...
ECR_REPOSITORY=relink-backend
EC2_HOST=<ec2-public-ip-or-domain>
EC2_USER=ubuntu
EC2_SSH_PRIVATE_KEY=<private-key-that-can-ssh-to-ec2>
EC2_DEPLOY_DIR=/opt/relink/backend
```

`EC2_DEPLOY_DIR` is optional. The workflow defaults to `/opt/relink/backend`.
